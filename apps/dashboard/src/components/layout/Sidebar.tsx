...
import { FileText, HardDrive, Home, Server, Settings } from "lucide-react";
			...
			<nav className="flex-1 space-y-1">
				{navItems.map(({ to, icon: Icon, label }) => (
					<Link
						...
						activeProps={{
							className:
								"flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium bg-[var(--surface)] text-[var(--text-primary)]",
						}}
					>
						<Icon size={18} />
						{label}
