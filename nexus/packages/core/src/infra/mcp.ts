...
type McpServerConfig = z.infer<typeof mcpServiceConfigSchema>;
...
async function connectToServer(serverConfig: McpServerConfig): Promise<Client> {
  ...
  const sseTransport = new sse.SSEClientTransport(url, { requestInit });
...
function convertMcpTools(
  ...
  return mcpTools.map((mcpTool) => {
    ...
    return toolDefinition({
      name: namespacedName,
      description: mcpTool.description || `${serverId} tool: ${mcpTool.name}`,
