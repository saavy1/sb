...
export const k8sAdapter = {
	...
	async listPods(labelSelector?: string): Promise<Pod[]> {
		const namespace = config.K8S_NAMESPACE;
		try {
			return await client.listPods(namespace, labelSelector);
		} catch (error) {
			logger.error({ error, namespace, labelSelector }, "Failed to list pods");
			throw error;
		}
	},
	...
	async getPod(name: string): Promise<Pod | null> {
		const namespace = config.K8S_NAMESPACE;
		try {
			return await client.getPod(namespace, name);
		} catch (error) {
			if (isK8sError(error) && (error as K8sError).status === 404) {
				return null;
			}
			logger.error({ error, name, namespace }, "Failed to get pod");
			throw error;
		}
	},
};
