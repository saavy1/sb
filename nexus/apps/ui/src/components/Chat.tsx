...
export function Chat({ threadId: propThreadId, onThreadChange }: Props) {
  ...
  // Handle message click for selection (shift+click for range)
  const handleMessageClick = useCallback(
    (messageId: string, event: React.MouseEvent) => {
      // Filter to only user/assistant messages for selection
      const selectableMessages = messages.filter((m) => m.role !== "system");

      if (event.shiftKey && lastClickedIdRef.current) {
        // Range select
        const lastIdx = selectableMessages.findIndex(
          (m) => m.id === lastClickedIdRef.current,
        );
        const currentIdx = selectableMessages.findIndex(
          (m) => m.id === messageId,
        );
        ...
        if (lastIdx !== -1 && currentIdx !== -1) {
          ...
          const rangeIds = selectableMessages
            .slice(start, end + 1)
            .map((m) => m.id);
  ...
  // Copy selected messages (or all if none selected) to clipboard
  const handleCopyMessages = useCallback(async () => {
    ...
    const formatted = messagesToCopy
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="relative flex-1 overflow-y-auto p-3 md:p-4">
        {/* Copy/selection controls - only show when there are messages */}
        {messages.length > 0 && (
          <div className="absolute right-3 top-3 z-10 flex items-center gap-2 md:right-4 md:top-4">
            {selectedIds.size > 0 && (
              <button
                ...
                className="flex items-center gap-1 rounded-md bg-zinc-800 px-2 py-1.5 text-xs text-zinc-400 transition-colors hover:bg-zinc-700 hover:text-zinc-300"
            ...
            <button
              ...
              className="flex items-center gap-1.5 rounded-md bg-zinc-800 px-2 py-1.5 text-xs text-zinc-400 transition-colors hover:bg-zinc-700 hover:text-zinc-300 md:px-2.5"
              title={selectedIds.size > 0 ? "Copy selected" : "Copy all"}
            >
              {copied ? (
                <>
                ...
                <>
                  ...
                  <span className="hidden sm:inline">
                    {selectedIds.size > 0 ? "Copy" : "Copy all"}
                  </span>
                </>
              )}
            </button>
          ...
          {messages.map((message) => {
            ...
            return (
              <div
                ...
                className={`flex cursor-pointer gap-3 rounded-lg p-1 -m-1 transition-colors ${
                  message.role === "user" ? "justify-end" : "justify-start"
                } ${isSelected ? "bg-emerald-500/10 ring-1 ring-emerald-500/30" : "hover:bg-zinc-800/50"}`}
              >
      ...
      <form
        ...
        className="border-t border-zinc-800 p-3 md:p-4"
      >
