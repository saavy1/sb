...
async function handleCreate(interaction: ChatInputCommandInteraction) {
  ...
  const nameResult = serverNameSchema.safeParse(
    interaction.options.getString("name", true)
  );
  const modpackResult = modpackSlugSchema.safeParse(
    interaction.options.getString("modpack", true)
  );
  if (!nameResult.success || !modpackResult.success) {
    const message = [
      ...(!nameResult.success ? nameResult.error.issues.map((i) => `name: ${i.message}`) : []),
      ...(!modpackResult.success ? modpackResult.error.issues.map((i) => `modpack: ${i.message}`) : []),
    ].join("\n");
  ...
  if (error || !data) {
    await interaction.editReply(`Failed to create server: ${String(error?.value) || "Unknown error"}`);
    return;
  }

  const server = data as GameServer;
...
async function handleList(interaction: ChatInputCommandInteraction) {
  ...
  if (error || !data) {
    await interaction.editReply(`Failed to list servers: ${String(error?.value) || "Unknown error"}`);
    return;
  }

  const servers = data as unknown as GameServer[];
  if (servers.length === 0) {
...
async function handleStartWithMenu(interaction: ChatInputCommandInteraction) {
  ...
  collector.on("collect", async (menu) => {
    ...
    const { error: startError } = await gameServers({ name: serverName }).start.post();
    if (startError) {
      await interaction.editReply({ content: `Failed to start: ${startError.value}`, components: [] });
    } else {
...
async function handleStopWithMenu(interaction: ChatInputCommandInteraction) {
  ...
  collector.on("collect", async (menu) => {
    ...
    const { error: stopError } = await gameServers({ name: serverName }).stop.post();
    if (stopError) {
      await interaction.editReply({ content: `Failed to stop: ${stopError.value}`, components: [] });
    } else {
